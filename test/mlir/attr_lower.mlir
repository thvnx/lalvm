...
// `@positive` is fine here. Fed as MLIR (no Ada emitter yet).

// RUN: %lalvm --emit=llvm %s | %FileCheck %s

module {
  // CHECK-LABEL: define i32 @_ada_first_attr
  ada.subp @first_attr(%lo: i32, %hi: i32) -> !ada.qual<i32, @positive> {
  ...
  ada.subp @last_attr(%lo: i32, %hi: i32) -> !ada.qual<i32, @positive> {
    %r = ada.range %lo, %hi : !ada.range<i32, @positive>
