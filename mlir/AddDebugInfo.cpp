//===- AddDebugInfo.cpp - Collect Ada enum metadata for DWARF -------------===//
//
...
// info emission for enumeration types. Emission is split into two stages
// because MLIR 21 lacks DIEnumeratorAttr and retainedTypes on
// DICompileUnitAttr, so the final DWARF construction must go through LLVM's
// native DIBuilder API after MLIR-to-LLVM IR translation.
//
// Pipeline overview (see lalvm.cpp::dumpLLVMIR for the full sequence):
//
//   lalvm.cpp::dumpLLVMIR owns a SmallVector<AdaEnumInfo> enumInfos and
//   passes it by reference into applyLoweringPasses, which runs:
//
//     AddAdaDebugInfoPass(enumInfos)  -> populate enumInfos from ada.type ops;
//                                       no IR mutations
//     LowerToLLVMPass                 -> erase ada.type ops
//
//   After translateModuleToLLVMIR, lalvm.cpp::attachEnumDebugInfo consumes
